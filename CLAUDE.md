...
- **ACTIVE RULES (2026-07-14 v5, `GT_MANUAL_DRAW=1`, now the runtime default)**: No automatic turn draw before the third Fortune. Each player banks **Action Points** (gain 1 on each own turn, cap 3); every primary action, including MAGIC, costs 1. Reactions and modal/post-move continuations cost 0. After an action/react chain, the same player may act again while points remain. **REST** is the existing rewarded no-action turn end: it costs 0 AP, banks the full pool, gives +1 mana, draws the current Fortune rate, and offers a Handshake. Once any AP is spent, REST is unavailable and **PASS** becomes the 0-cost, no-effect turn ender. Two consecutive RESTs seal a Handshake; payout remains both players +1 mana and draw 1. The Fortune economy progresses 1/1 → 2/2 after turn 25 → 3/3 after turn 50, then remains 3/3. After the Fortune at the end of turn 75, every turn also draws 1 automatically; an empty deck takes escalating 10/20/30... fatigue. `GT_MANUAL_DRAW=0` retains legacy compatibility rules.
