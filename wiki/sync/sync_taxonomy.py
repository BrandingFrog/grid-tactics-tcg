...
RULES_PAGES: dict[str, str] = {
    ...
    "Graveyard": (
        "The '''Graveyard''' is one of two discard piles in "
        "[[Grid Tactics TCG]]. It holds cards that have been played and "
        "resolved during the game.\n"
        "\n"
        "== Definition ==\n"
        "The graveyard is an ordered, face-up pile of cards associated "
        "with each player. Cards in the graveyard are public information "
        "and may be inspected by either player at any time.\n"
        "\n"
        "== When Cards Enter the Graveyard ==\n"
        "A card enters the graveyard under the following conditions:\n"
        "\n"
        "* '''Magic cards''' -- Placed in the graveyard immediately after "
        "their effect resolves.\n"
        "* '''React cards''' -- Placed in the graveyard immediately after "
        "their effect resolves.\n"
        "* '''Minion death''' -- When a minion's HP reaches 0, its card "
        "enters the owner's graveyard, '''provided''' the minion was "
        "originally played from the deck (i.e. <code>from_deck=True</code>).\n"
        "* '''Sacrifice''' -- When a minion is [[Sacrifice|sacrificed]] at "
        "the opponent's back row, its card enters the owner's graveyard "
        "(same <code>from_deck</code> condition).\n"
        "\n"
        "== When Cards Do NOT Enter the Graveyard ==\n"
        "* '''Conjured minions''' -- Minions summoned via [[Conjure]] have "
        "<code>from_deck=False</code>. When they die, they are removed "
        "from the game entirely and do not enter any pile.\n"
        "* '''Discard costs''' -- Cards removed from hand as a cost (e.g. "
        "[[Cost]]: [[Discard]] any Robot) enter the [[Exhaust Pile]] "
        "instead.\n"
        "\n"
        "== Graveyard vs Exhaust ==\n"
        "The graveyard and [[Exhaust Pile]] are distinct zones. The "
        "graveyard contains cards that were ''played''; the exhaust pile "
    ...
    "Exhaust Pile": (
        ...
        "\n"
        "* '''[[Cost]]: [[Discard]]''' -- Cards with a discard cost (e.g. "
        "[[Card:RGB Lasercannon|RGB Lasercannon]]: ''Cost: Discard any "
        "2 Robots'') send the sacrificed hand cards to the exhaust pile.\n"
        "\n"
        "== Exhaust vs Graveyard ==\n"
        "The exhaust pile is '''not''' the [[Graveyard]]. Cards exhausted "
        "as costs were never ''played'' -- they were ''paid''. This "
        "distinction matters for card effects that reference played cards "
        "or graveyard contents.\n"
        "\n"
        "== See Also ==\n"
        "* [[Graveyard]]\n"
        "* [[Cost]]\n"
