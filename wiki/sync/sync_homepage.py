...
def main_page_wikitext(cards_dir: Path | None = None) -> str:
    ...
    """
    # Card of the Day — full-art in-game card frame (shared cf2 replica)
    cotd = _pick_card_of_the_day(cards_dir)
    if cotd:
        from sync.sync_cards import build_cf2_card_html

        card_name = cotd.get("name", "Unknown")
        card_html = build_cf2_card_html(cotd, link_page=f"Card:{card_name}")
        cotd_section = "== Card of the Day ==\n" + card_html + "\n\n"
    else:
        cotd_section = ""
