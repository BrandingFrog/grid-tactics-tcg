...
Last session: 2026-04-18T18:48:00.000Z — Completed 14.7-02 (3-phase turn state machine wired; infrastructure for 14.7-03 triggered effects). Previous: 2026-04-18T18:15:00.000Z — Completed 14.7-01 standalone (Deferred magic resolution via cast_mode originator; fixes Acidic-Rain-vs-Prohibition).
Stopped at: Plan 14.7-02 SHIPPED. Two commits (d2e6303 Task 1 feat + d842ab7 Task 2 feat) pushed to master; Railway auto-deployed. TurnPhase extended append-only (START_OF_TURN=2, END_OF_TURN=3). ReactContext IntEnum + GameState.react_context + react_return_phase fields with full round-trip. resolve_react_stack dispatches on react_return_phase (None default = legacy AFTER_ACTION byte-identical). 4 phase-transition helpers: enter_start_of_turn / enter_end_of_turn (placeholders until 14.7-03), close_start_react_and_enter_action, close_end_react_and_advance_turn. _close_end_of_turn_and_flip helper deduplicates turn-advance tail between main path and pending_death resume path. All 6 `phase=TurnPhase.REACT` sites in action_resolver.py tag react_context=AFTER_ACTION / react_return_phase=ACTION. legal_actions returns () for START/END phases; events.py auto-advance loop routes directly to helpers (resolve_action does NOT accept START/END). Safety counter=50 guards against infinite loops. Rule-1 fix: discard-flip now correctly on main path too. 16 new tests; 0 regressions (6 pre-existing baseline failures unchanged: 4 LEAP game_loop, 1 spectator, 1 RL self_play). view_filter.py byte-unchanged. Next: 14.7-03 (ON_START_OF_TURN / ON_END_OF_TURN triggered effects + card JSON retagging) — hook points in enter_start_of_turn / enter_end_of_turn already in place.
