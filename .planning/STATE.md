...
- Plus 11 socket-emit assertions migrated from `state_update` / `sandbox_state` to `engine_events` across `test_event_serialization.py`, `test_game_flow.py`, `test_events.py`, `test_sandbox_events.py` (these are separate from the last_trigger_blip M7 count — they're "mechanical rename" migrations triggered by the snapshot-path deletion).

Commits: `ca5fc39` (feat Task 1) + `1c762ea` (refactor Task 2) + docs commit pending (this metadata update).

Hook points for plan 06 (visual UAT scenarios):
- Spell stage must open correctly for all 6 ReactContext variants; the paladin-heal scenario is the primary regression-test anchor (HP should stay at 30 for ≥1500ms before ticking to 32).
- Pre-existing flaky failures in test_react_stack start-of-turn triggers + test_game_loop LEAP bug surface under strict mode — several target paladin-like flows that would be UAT scenarios; may need addressing as a prerequisite.
- The 4 pending_modal kinds that don't emit EVT_PENDING_MODAL_OPENED today (conjure_deploy, revive_place, magic_cast_originator, post_move_attack) are still gapped and might surface as visual artifacts in UAT; handler is generic so engine emission is the only gap.
