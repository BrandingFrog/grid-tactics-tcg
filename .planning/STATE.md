...
status: in_progress
stopped_at: "Phase 14.7 Plan 05 SHIPPED. Simultaneous ON_START_OF_TURN / ON_END_OF_TURN triggers now enqueue into pending_trigger_queue_{turn,other} priority queues instead of resolving inline (spec §7.2 turn-player-first). Engine changes: (1) PendingTrigger frozen dataclass + 3 GameState fields (queue_turn, queue_other, picker_idx) + full to_dict/from_dict round-trip. (2) fire_start_of_turn_triggers / fire_end_of_turn_triggers rewritten to enqueue via _enqueue_turn_phase_triggers + call drain_pending_trigger_queue. (3) drain_pending_trigger_queue: turn queue drains first; >=2 entries sets picker_idx (opens modal); singleton auto-resolves via _resolve_trigger_and_open_react_window (shared helper); 0 falls through to other queue. (4) _resolve_trigger_and_open_react_window resolves one effect using captured_position (SELF_OWNER survives minion move/death — true fizzle re-validation lands in 14.7-06), pops from the correct queue, and opens a REACT window tagged with AFTER_START_TRIGGER / BEFORE_END_OF_TURN. (5) _apply_trigger_pick moves picked entry to front of queue then calls the shared helper (no double-pop). _apply_decline_trigger clears the entire remaining queue (fizzle-all escape hatch) and re-enters drain for the other queue. (6) **Drain-recheck hook in resolve_react_stack** — the critical Warning-8 ordering fix: the re-drain block sits AFTER _cleanup_dead_minions + is_game_over check, BEFORE the return_phase dispatch. test_two_triggers_second_fires_after_first regression test validates this ordering explicitly. (7) ActionType extended with TRIGGER_PICK=17 (reuses PLAY_CARD slot space with stride=GRID_SIZE, same trick as TUTOR_SELECT) + DECLINE_TRIGGER=18 (reuses PASS slot 1001). RL action space encode/decode wired; decode prioritizes trigger_picker (phase-agnostic gate). Server: enrich_pending_trigger_for_viewer mirrors 14.2 tutor asymmetric pattern — picker sees full queue_entries list; opponent sees only picker_idx + queue_length. Registered at all 8 events.py fanout sites. Client: syncPendingTriggerPickerUI + showTriggerPickerModal reuse renderDeckBuilderCard + .tutor-modal CSS verbatim (user directive 'we want to use existing modals'). Click emits TRIGGER_PICK(queue_idx=i); Skip emits DECLINE_TRIGGER. Sandbox god-mode always opens modal. _cleanup_dead_minions UNCHANGED — Death: priority migration is 14.7-05b (scope-fenced out). Commits: eeda694 (feat Task 1: dataclass + fields + slots) + 2cd7e66 (feat Task 2: engine rewire + drain + handlers + drain-recheck) + cc6ba84 (feat Task 3: server fanout + client modal). All pushed to master; Railway auto-deployed. Test posture: 546 engine + server tests pass (+14 new tests across 5 files); baseline failures unchanged (27 pre-existing; tensor engine + training + RL action space REVIVE_PLACE/DEATH_TARGET_PICK unhandled — all pre-existing issues not introduced by 14.7-05). Auto-fixed 3 deviations during execution: (a) double-pop bug in _apply_trigger_pick (refactored to move-to-front), (b) RL action space didn't know TRIGGER_PICK/DECLINE_TRIGGER (added encode+decode), (c) test_action_type_count off-by-2 (bumped 17→19). Hook points ready for 14.7-05b (Death: migration — same queue pattern, same drain-recheck hook insertion point), 14.7-06 (fizzle — insert liveness check at top of _resolve_trigger_and_open_react_window), 14.7-07 (OPPONENT_START_OF_TURN / OPPONENT_END_OF_TURN react conditions match against react_context tags that are already flowing), 14.7-09 (picker modal has anchor payload for source→target blip animations)."
...
Last activity: 2026-04-19 — Completed 14.7-05-PLAN.md (Simultaneous-trigger priority queue + modal picker for START/END-of-turn triggers. PendingTrigger dataclass + pending_trigger_queue_{turn,other} + pending_trigger_picker_idx on GameState. fire_{start,end}_triggers enqueue instead of resolve inline; drain_pending_trigger_queue walks turn-queue-first, auto-resolves singletons, opens modal for 2+. _resolve_trigger_and_open_react_window shared pop-and-open helper. Drain-recheck hook in resolve_react_stack runs AFTER cleanup + game-over, BEFORE return_phase dispatch — test_two_triggers_second_fires_after_first regression validates. TRIGGER_PICK / DECLINE_TRIGGER reuse PLAY_CARD[0:N] + PASS slots (same trick as 14.2 tutor). Server: enrich_pending_trigger_for_viewer asymmetric filter (picker sees full options; opponent sees picker_idx + count only). Client: syncPendingTriggerPickerUI + modal reuses renderDeckBuilderCard + .tutor-modal CSS verbatim. _cleanup_dead_minions UNCHANGED — 14.7-05b migration.). Previous: 14.7-08 (Melee two react windows), 14.7-04 (Summon compound react windows), 14.7-03 (Start/End/Summon triggered effects pipeline), 14.7-02 (3-phase turn state machine), 14.7-01 (Deferred magic resolution).
