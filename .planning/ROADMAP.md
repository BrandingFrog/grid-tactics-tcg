...
**Goal**: The game's turn loop implements the priority / phase / stack rules in `docs/rules/turn-structure.md` end-to-end — a turn has three distinct phases (Start / Action / End) each with its own react window, compound actions open multiple react windows, magic spells defer their effects until the react chain resolves so Prohibition can negate them, simultaneous effects resolve turn-player-first with a modal card-picker for multiple same-player triggers, and effects whose target is invalid at resolution time fizzle silently.
**Depends on**: Phase 14.6
**Requirements**: TURN-01 through TURN-18 (18 requirements derived from `docs/rules/turn-structure.md`; to be added to REQUIREMENTS.md in plan 14.7-10)
**Success Criteria** (what must be TRUE):
